        ...
        <S.ContentWrapper key={idx} target_nth={target_nth!} ratio={ratio!} onClick={() => navigateToPage(e)}>
          {category.map((elem, inner_idx) =>
            elem[1] === "request" ? (
              ...
              <InviteAcceptButton memberId={Number(memberId)} studyId={Number(studyId)} key={inner_idx} />
            ) : elem[1] === "user_invite" ? (
              <InviteAcceptButton memberId={Number(memberId)} studyId={e.id} key={inner_idx} />
            ) : elem[1] === "remove" ? (
              <RemoveProblemButton key={inner_idx} idx={idx + 1} />
            ) : elem[1] === "rank" ? (
              <div key={inner_idx}>{idx + 1}</div>
            ) : elem[1] === "capacity" ? (
              <div key={inner_idx}>
                ( {e.studyMember}/{e.capacity} )
              </div>
            ) : (
